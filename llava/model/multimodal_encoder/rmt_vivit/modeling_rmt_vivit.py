...
def frac_gradient(t, frac = 1.): # paper cogview
    if frac == 1.:
        return t
...
class RMTVivitModel(VivitPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config
        ...
        self.read_memory_tokens = nn.Parameter(torch.zeros(self.num_memory_tokens, self.config.hidden_size))
        nn.init.normal_(self.read_memory_tokens, std=0.02)
    ...
    def forward(
        ...
        # set up memories
        embedding_output = frac_gradient(embedding_output, self.embe)
        batch_size, num_frames, num_channels, height, width = pixel_values.shape
        write_memories = self.init_memory(batch_size)
        ...
        if read_memories is not None:
            if read_memories.ndim == 2:
                read_memories = repeat(read_memories, 'n d -> b n d', b = batch_size)
            ...
            read_mem_length = self.num_memory_tokens
            read_memories = repeat(self.read_memory_emb, 'n d -> b n d', b = batch_size)
        else:
            read_mem_length = 0
            read_memories = embedding_output[:, 0:0]
        embedding_output, ps = pack([read_memories, embedding_output, write_memories], 'b * d')
