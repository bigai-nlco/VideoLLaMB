...
def apply_chunking_to_forward(
    ...
    return forward_fn(*input_tensors, last_norm=last_norm)
...
class RobertaAttention(nn.Module):
    def __init__(self, config, layer_index=None):
        ...
        self.output = RobertaSelfOutput(config, layer_index=layer_index)
...
class RobertaEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.config.gradient_checkpointing = config_yaml['use_checkpoint']
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            raise ValueError("You cannot secify both input_ids and inputs_embeds at the same time")
