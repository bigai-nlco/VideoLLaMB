...
from .heads import Pooler, ITMHead, MLMHead
from .util import state_dict_data_parallel_fix



with open('llava/model/multimodal_encoder/egovlp/EgoNCE_MLM_ITM_Config.yml') as f:
...
class FrozenInTime(BaseModel):
    def __init__(self,
        ...
        if ('MLM' in self.task_names or 'ITM' in self.task_names):
            ...
            self.txt_model.NUM_FUSE_BLOCK = self.video_model.NUM_FUSE_BLOCK=self.num_fuse_block
            self.txt_model.DIM_IMG=self.config["input_image_embed_size"]
            ...
            self.cross_modal_video_pooler = Pooler(self.config["hidden_size"])
            self.cross_modal_video_pooler.apply(init_weights)
            self.cross_modal_text_pooler = Pooler(self.config["hidden_size"])
        ...
        if 'ITM' in self.task_names:
            self.itm_score = ITMHead(self.config["hidden_size"] * 2)
    ...
    def forward(self, data, n_embeds, v_embeds, allgather, n_gpu, args, config, loss_egonce, gpu, return_embeds=True, task_names='EgoNCE_ITM_MLM'):
        ...
        if 'Feature_Extraction' in task_names:
            ...
            video_embeddings = self.compute_video(data)
