...
from .configuration_video import RMTLanguageBindVideoConfig, RMTCLIPVisionConfig, RMTCLIPTextConfig
...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, config: RMTLanguageBindVideoConfig):
        super().__init__()
    ...
    def forward(
        ...
        if self.add_time_attn:
            ...
            if t != 1:
                ...
                hidden_states = rearrange(hidden_states, '(b t) n d -> (b n) t d', b=1)
...
class CLIPPreTrainedModel(PreTrainedModel):
    ...
    config_class = RMTLanguageBindVideoConfig
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, RMTLanguageBindVideo):
            nn.init.normal_(
...
class CLIPEncoder(nn.Module):
    ...
    def __init__(self, config: RMTLanguageBindVideoConfig):
        ...
        self.emb_gradient_frac = 0.1
        self.num_memory_tokens = 128
        self.read_memory_emb = nn.Parameter(torch.zeros(self.num_memory_tokens, self.config.hidden_size))
        nn.init.normal_(self.read_memory_emb, std=0.02)
        self.memory_tokens = nn.Parameter(torch.randn(self.num_memory_tokens, self.config.hidden_size))
        nn.init.normal_(self.memory_tokens, std=0.02)
        # in the paper, they actually also use the previous write memories for the next write memories
        self.add_write_to_next_write_mem = False
        self.next_write_mem_stop_grad = True
        # allow for attending to raw read memory positional embeddings on first step
        # hack to make it onnx-able and should not hurt
        self.always_have_read_memories = True
    ...
    def forward(
        ...
        # # set up memories for causal language model: read_memory + x + write_memory
        # B = hidden_states.shape[0]
        # write_memories = self.init_memory(B)
        # if read_memories is not None and self.add_write_to_next_write_mem:
        ...
        # else:
        #     read_mem_length = 0
        #     read_memories = hidden_states[:, 0:0]
        # hidden_states, ps = pack([read_memories, hidden_states, write_memories], 'b * d')
        ...
        # set up memories for encoder: read_memory + x
        B = hidden_states.shape[0]
        # prepare read memories
        if read_memories is not None:
            if read_memories.ndim == 2:
        ...
        # read_memories, hidden_states, write_memories = unpack(hidden_states, ps, 'b * d')
        ...
        if not return_dict:
            # return tuple(v for v in [hidden_states, write_memories, encoder_states, all_attentions] if v is not None)
            return tuple(v for v in [hidden_states, read_memories, encoder_states, all_attentions] if v is not None)
...
class CLIPTextTransformer(nn.Module):
    def __init__(self, config: RMTCLIPTextConfig):
        super().__init__()
        self.config = config
    ...
    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=RMTCLIPTextConfig)
    def forward(
        self,
...
class CLIPTextModel(CLIPPreTrainedModel):
    config_class = RMTCLIPTextConfig
...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, config: RMTCLIPVisionConfig):
        super().__init__()
        self.config = config
        embed_dim = config.hidden_size
    ...
    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=RMTCLIPVisionConfig)
    def forward(
        self,
        ...
        # all_hidden_states = [[] for i in range(self.config.num_hidden_layers)]
        if T == 1:
        ...
        raise NotImplimentatedError("TODO: Encoder RMT ")
        # can't segment video sequence by semantics
        ...
        for segment, hidden_state in enumerate(hidden_states):
            ...
            # all_last_hidden_states.append(encoder_outputs[0])
            memories = encoder_outputs[1]
            # if segment == hidden_states.shape[0] - 1:
            #     last_hidden_states = encoder_outputs[2]
            # for i, layer_hidden_state in enumerate(encoder_output[2]):
            #     all_hidden_states[i].append(rearrange(layer_hidden_state, '(b t) n c -> b t n c', b=1))
        ...
        if not return_dict:
            # return (last_hidden_state, pooled_output) + encoder_outputs[1:]
            return (last_hidden_state, pooled_output) + (last_hidden_states, )
...
class CLIPVisionModel(CLIPPreTrainedModel):
    config_class = RMTCLIPVisionConfig
...
class RMTLanguageBindVideo(CLIPPreTrainedModel):
    config_class = RMTLanguageBindVideoConfig
    ...
    def __init__(self, config: RMTLanguageBindVideoConfig):
        ...
        if not isinstance(config.text_config, RMTCLIPTextConfig):
            raise ValueError(
                "config.text_config is expected to be of type RMTCLIPTextConfig but is of type"
                f" {type(config.text_config)}."
            )

        if not isinstance(config.vision_config, RMTCLIPVisionConfig):
            raise ValueError(
                "config.vision_config is expected to be of type RMTCLIPVisionConfig but is of type"
    ...
    @replace_return_docstrings(output_type=CLIPOutput, config_class=RMTLanguageBindVideoConfig)
    def forward(
        self,
