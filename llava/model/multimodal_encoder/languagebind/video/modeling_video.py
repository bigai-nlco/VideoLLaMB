...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, config: LanguageBindVideoConfig):
        ...
        # self.add_time_attn = False # DEBUG
        if self.add_time_attn:
            ...
            self.t = 8
            self.temporal_embedding = nn.Parameter(torch.zeros(1, 8, config.hidden_size))
            nn.init.normal_(self.temporal_embedding, std=config.hidden_size ** -0.5)

            # self.temporal_embedding_ = nn.Parameter(torch.zeros(1, config.num_frames, config.hidden_size))
            # nn.init.normal_(self.temporal_embedding_, std=config.hidden_size ** -0.5)
    ...
    def forward(
        ...
        if self.add_time_attn:
            ...
            if t != 1:
                ...
                # hidden_states = hidden_states + self.temporal_embedding_[:, :t, :]
