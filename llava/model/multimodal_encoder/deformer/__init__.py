...
from .deformer_modeling import DeformableVisionModel

class DeformableImageTower(nn.Module):
    def __init__(self, image_tower, args, delay_load=False, cache_dir='./cache_dir'):
        ...
        if not delay_load:
            ...
            self.cfg_only = DeformableDetrConfig.from_pretrained(self.image_tower_name, cache_dir=self.cache_dir)
            self.image_processor = DeformableDetrImageProcessor.from_pretrained(self.image_tower_name)
            self.image_tower = DeformableVisionModel(self.cfg_only)
    ...
    def load_model(self, device_map=None):
        ...
        self.image_processor = DeformableDetrImageProcessor.from_pretrained(self.image_tower_name)
        self.image_tower = DeformableVisionModel.from_pretrained(self.image_tower_name, device_map=device_map)
