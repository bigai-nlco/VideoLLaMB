...
class TransformerProjector(nn.Module):
    def __init__(self, config, depth):
        ...
        self.t, self.h, self.w = 4, 12, 12
        self.pooler = nn.AdaptiveAvgPool2d((self.h, self.w))
        self.st_pooler = nn.AdaptiveAvgPool3d((self.t, self.h, self.w))
    ...
    def forward(
        ...
        # # h = int(w * 224 // 224)
        # # hidden_states = rearrange(hidden_states, 'bt (h w) d -> bt d h w', bt=bt, h=h, w=w, d=d)
        # # hidden_states = self.pooler(hidden_states)
        # # hidden_states = rearrange(hidden_states, 'bt d h w -> bt (h w) d', bt=bt, h=self.h, w=self.w, d=d)
        ...
        if t == 1:
            ...
            hidden_states = rearrange(hidden_states, 'b t (h w) d -> b d t h w', b=b, t=t, h=h, w=w, d=d)
            hidden_states = self.st_pooler(hidden_states)
            hidden_states = rearrange(hidden_states, 'b d t h w -> b (t h w) d', b=b, t=self.t, h=self.h, w=self.w, d=d)
