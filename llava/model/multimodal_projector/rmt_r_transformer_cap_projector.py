...
class RMTRTransformerProjector(nn.Module):
    ...
    def forward(
        ...
        if t == 1:
            ...
            boundaries = segment(cls_states, k=7)
            # print(boundaries)
            ...
            for bi in boundaries:
                # sample
                steps = torch.linspace(index, bi, min(4, bi-index+1), dtype=torch.int).to(hidden_states.device)
                ...
                all_hidden_states.append(last_hidden_states.clone().detach())
