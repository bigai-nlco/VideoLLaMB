...
from .self_retriever import TransformerRetriever
...
class TransformerProjector(nn.Module):
    ...
    def forward(
        ...
        if output_attentions: 
            ...
            return proj_hidden_states, read_memories, hidden_states
...
class RMTRTransformerProjector(nn.Module):
    def __init__(self, config, depth):
        super().__init__()
        self.config = config
        self.projector = TransformerProjector(config, depth)
        self.retrieval = TransformerRetriever(config)

        # self.pooler = nn.AvgPool2d(kernerl_size=4, stride=4)
        self.h, self.w = 12, 12
    ...
    def forward(
        ...
        if t == 1:
            ...
            # hidden_states = rearrange(hidden_states, '(b t) (h w) d -> (b t) d h w', b=b, t=t, h=h, w=w, d=d)
            # hidden_states = self.pooler(hidden_states)
            # hidden_states = rearrange(hidden_states, '(b t) d h w -> (b t) (h w) d', b=b, t=t, h=8, w=8, d=d)
            ...
            for bi in boundaries:
                # prevent from exceed context length
                ...
                read_memories = self.retrieval(
                    read_memories,
                    encoder_hidden_states = torch.cat(memory_cache, dim=1)
                )
        
        if all_last_hidden_states != []:
            return last_hidden_states, all_last_hidden_states
        else:
